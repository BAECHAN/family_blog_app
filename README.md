...
1. **로그인**( 담당자 : 지형님)
   ...
   - 회원가입 : Oauth 및 관리자권한을 가진 계정이 수동등록 후 초대코드 혹은 id/pw 전달
     - Oauth : 카카오 혹은 네이버 , 구글 로그인
     - 관리자 계정이 수동등록
     - 직접 회원가입 ( 선택 ) : 회원가입 폼 필요
   - 첫 로그인 시 [ 가족만들기 / 가족들어가기 ] 하나를 선택 할 수 있도록 ( 만약 초대코드를 활용한다면 스킵 가능 )
     - 선택 후 이어서 자신의 프로필 등록하기 ( 슬라이드 형식의 설문조사 느낌으로 )
   - 인증 및 인가

2. **마이페이지**( 담당자 : 지형님)

   - 회원정보 수정 ( 사진, 생년월일, 좋아하는것, 자주듣는노래, _받고싶은선물_ 등 일부 정보는 '가계도'에서 상세 클릭 시 보이게함)
   - 비밀번호 변경
